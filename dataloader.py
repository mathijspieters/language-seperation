...
def load_hatespeech(dataloader_params, use_balanced_loader=False, allowed_words=None):

    df = utils.process_hatespeech()
    ...
    vocab.build(vocab_size=2000)

    vocab.num_labels = len(labels_map)
    vocab._i2l = labels_map.keys()
    vocab._l2i = labels_map

    tokenizer = lambda x: vocab.sentence2IDs(x)

    labels_train = list(df.loc[df.splitset_label == 'train', 'label'].map(labels_map).astype(int))
    data_train = [tokenizer(preprocess(str(s))) for s in df.loc[df.splitset_label == 'train', 'tokens']]
    training_set = dataset.Dataset(data_train, labels_train)

    labels_test = list(df.loc[df.splitset_label == 'test', 'label'].map(labels_map).astype(int))
    data_test = [tokenizer(preprocess(str(s))) for s in df.loc[df.splitset_label == 'test', 'tokens']]

    test_set = dataset.Dataset(data_test, labels_test)
...
def load_hatespeech_tokens(dataloader_params, use_balanced_loader=False, allowed_words=None):
    ...
    labels_train = list(df.loc[df.splitset_label == 'train', 'label'].map(labels_map).astype(int))
    data_train = [tokenizer(preprocess(str(s))) for s in df.loc[df.splitset_label == 'train', 'tokens']]
    data_train_tokens = [[tokenizer_words(w) for w in vocab.split_sentence(preprocess(str(s)))] for s in
                         df.loc[df.splitset_label == 'train', 'tokens']]
    ...
    labels_test = list(df.loc[df.splitset_label == 'test', 'label'].map(labels_map).astype(int))
    data_test = [tokenizer(preprocess(str(s))) for s in df.loc[df.splitset_label == 'test', 'tokens']]
    data_test_tokens = [[tokenizer_words(w) for w in vocab.split_sentence(preprocess(str(s)))] for s in
                        df.loc[df.splitset_label == 'test', 'tokens']]

    test_set = dataset.Dataset_tokens(data_test, data_test_tokens, labels_test)
